<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link
      href="https://cdn.jsdelivr.net/npm/remixicon@3.4.0/fonts/remixicon.css"
      rel="stylesheet"
    />
    <link rel="stylesheet" href="style.css" />
    <title>CareCompass</title>
  </head>
            <div class="containerh">
            <nav class="navbar navbar-expand-lg bg-lightblue shadow-lg p-0">
                    <div class="nav__logo">CareCompass</div>
                    <ul class="nav__links">
                    <li class="link"><a href="hospital_admin.php">Hospital</a></li>
                    <li class="link"><a href="medicine_admin.php">Medicine</a></li>
                    <li class="link"><a href="donor_admin.php"> Blood Donor</a></li>
                    <li class="link"><a href="logout.php">Log Out</a></li>

                    
                    </ul>
            </nav>
            </div>
</html>
